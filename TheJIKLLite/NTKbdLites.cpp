...
int DECLSPEC TurnKeyboardLight(HANDLE hKbdDev, UINT LedFlag, BOOL state)
{
...
HANDLE DECLSPEC OpenKeyboardDevice(int *ErrorNumber)
{
	...
	*LocalErrorNumber = 0;
	...
	hndKbdDev = CreateFile(TEXT("\\\\.\\Kbd"), GENERIC_WRITE, 0, NULL,	OPEN_EXISTING,	0,	NULL);
...
//
HANDLE DECLSPEC FlashKeyboardLightInThread(UINT LightFlag, int Duration, LPCWSTR EventName)
{
	DWORD ThreadId;
